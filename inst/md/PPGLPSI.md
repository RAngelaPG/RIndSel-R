...
* The variance of $I_P=b'_Py$  is equal to the covariance between $I_P=b'_Py$ and $H=w'a$. As $K_M=K^2_M$ and $K'_MP=PK_M$ and $b'P=w'G$ then $\sigma^2_{I_P}=b'_P P b_P=b'K'_M b=b'PK^2_M b=w'Gb_P=\sigma_{HI_P}$.
* The maximized correlation between $H$ and $I_P=b'_Py$ is equal to $\rho_{HI_P}=\frac{\sigma_{I_P}}{\sigma_H}$. In point 4 of this subsection, we showed that $\sigma_{HI_P}=\sigma^2_{I_P}$ then
  $\rho_{HI_P}=\frac{w'Gb_P}{\sqrt{w'Gw}\sqrt{b'_P P b_p}}=\sqrt{\frac{b'_P P b_p}{w'Gw}}$.  
