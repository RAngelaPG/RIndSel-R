SmithIndex <-function (file.dat = NULL, file.wgt = NULL, selval = 5, design = "lattice", 
    ...
    BSmith <- as.vector(solve(MVP) %*% MVG %*% theta)
	...
	Rsel<-PlotResp(VIDS,"Smith",selval)
	#Rsel<-ks*sqrt(VIDS)
	#H2<-(t(BSmith)%*%MVG%*%BSmith)/(t(BSmith)%*%MVP%*%BSmith)
    ...
    coefSmith<-as.data.frame(t(BSmith))
	colnames(coefSmith)<-colnames(MVG)
    ...
    cat("\n", paste("COEFFICIENTS OF SMITH INDEX", sep = " "),"\n", file = out, append = T)
    print.char.matrix(round(coefSmith, 3), file = out, col.names = T, append = T)    	    	
