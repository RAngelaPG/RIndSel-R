KNGSIIndex <- function(file.dat=NULL,file.wgt=NULL,selval=5,design="lattice",corr=FALSE,method=c("vanraden","classic"),out="out.txt",outcsv="out.csv",rawdata=TRUE,markers=NULL,one.env=T, block.ex, softR, file.covG){
    if(rawdata==TRUE){
    traits <<- NULL
    traits2 <<- NULL
    covG <<- NULL
  ...
  rsin=rsi[!is.na(rsi)]  
  EigVecKNGSI <- IndCoef(theta=theta,rest=rsin,Gamma=MVM,G=MVG,evd=F)
  EigVecKNGSI <- EigVecKNGSI/as.vector(sqrt(t(EigVecKNGSI)%*%EigVecKNGSI))
  ebv <- gebv$ebv
  YKNGSI <<- scale(ebv)%*%EigVecKNGSI
  rnames <- rownames(YKNGSI)
  vGb <- t(theta)%*%MVM%*%EigVecKNGSI
  VIDS <- t(EigVecKNGSI)%*%MVP%*%EigVecKNGSI
  VDCG <- t(theta)%*%MVM%*%theta
  vGv <- sqrt(abs(VDCG))
  bPb <- sqrt(abs(VIDS))
  corrAnalysis <- min(0.9999,ifelse(corr!=TRUE,vGb/(vGv%*%bPb),vGb))  
  selected <- YKNGSI>=quantile(YKNGSI,1-selval/100)
  selYKNGSI <- YKNGSI[selected]
  ord <- order(selYKNGSI,decreasing=TRUE)
  selYKNGSI <- selYKNGSI[ord]
  ...
  MKNGSISI <- apply(selentry,2,mean)
  MKNGSIall <- apply(allmean,2,mean)
  GAINbyMEANS=MKNGSISI-MKNGSIall
  ks <-(100/selval)*(1/sqrt(2*pi))*exp((-qnorm(1-(selval/100))^2)/2)
  #ks <- if(selval==5) 2.063 else if(selval==10) 1.755 else 1.400
  KNGSIGain <- as.vector(ks*(MVM%*%EigVecKNGSI)/as.numeric(sqrt(VIDS)))
  selentry2 <- rbind(selentry,MKNGSISI,MKNGSIall,GAINbyMEANS,KNGSIGain)
  selKNGSI <- data.frame(selentry2,c(selYKNGSI,rep(NA,4)))
  colnames(selKNGSI)[ncol(selKNGSI)] <- "KNGSI index"
  ...
  rownames(selKNGSI)[(length(selYKNGSI)+1):(length(selYKNGSI)+4)] <- rnames
  allKNGSI <- data.frame(allmean,YKNGSI)
   colnames(allKNGSI)[ncol(allKNGSI)] <- "KNGSI index"
  cat("KNGSI SELECTION INDEX METHOD","\n",file=out)
  ...
  cat("\n\n","COVARIANCE BETWEEN THE KNGSI SELECTION INDEX AND THE BREEDING VALUE: ", vGb,"\n",file=out,append=T)
  cat("\n","VARIANCE OF THE KNGSI SELECTION INDEX:                               ", VIDS,"\n",file=out,append=T)
  cat("\n","VARIANCE OF THE BREEDING VALUE:                                      ", VDCG,"\n",file=out,append=T)
  cat("\n","CORRELATION BETWEEN THE KNGSI SELECTION INDEX AND THE BREEDING VALUE:", corrAnalysis,"\n",file=out,append=T)
  cat("\n\n",
      paste("VALUES OF THE TRAITS FOR SELECTED INDIVIDUALS AND THE VALUE OF THE KNGSI SELECTION, MEANS AND GAINS FOR", paste(selval,"%",
            sep =""),sep=" "),"\n",file=out,append=T)
  print.char.matrix(round(selKNGSI,2),file=out,col.names=T,append=T)
  cat("\n\n","VALUES OF THE TRAITS FOR ALL INDIVIDUALS AND THE VALUE OF THE KNGSI SELECTION INDEX","\n",file=out,append=T)
  print.char.matrix(round(allKNGSI,2),file=out,col.names=T,append=T)
  #file.show(out)
  write.csv(selentry,na="",file=outcsv)
  #write.csv(allKNGSI,file=paste("all",outcsv,sep=""))
  allprint=allKNGSI
